...
//        [[DDUserModule shareInstance] getUserForUserID:self.currentUserID Block:^(ZKUserEntity *user) {
            ZKPublicProfileViewController *public = [ZKPublicProfileViewController new];
//            public.user=user;
            [[ZKChattingMainViewController shareInstance] pushViewController:public animated:YES];
//        }];
    }
}
...
{
    // 获取气泡设置
    _leftConfig = [[ZKBubbleModule shareInstance] getBubbleConfigLeft:YES];
    _rightConfig = [[ZKBubbleModule shareInstance] getBubbleConfigLeft:NO];
...
- (CGSize)sizeForContent:(ZKMessageEntity*)message
{
    return CGSizeZero;
}
