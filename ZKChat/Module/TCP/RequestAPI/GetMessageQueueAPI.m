...
- (Analysis)analysisReturnData
{
    ...
    Analysis analysis = (id)^(NSData* data)
    {
        ...
        NSString *sessionID = [ZKUtil changeOriginalToLocalID:rsp.sessionId SessionType:sessionType];
        NSUInteger begin = rsp.msgIdBegin;
        NSMutableArray *msgArray = [NSMutableArray new];
        for (MsgInfo *msgInfo in [rsp msgList]) {
            ZKMessageEntity *msg = [ZKMessageEntity makeMessageFromPB:msgInfo SessionType:sessionType];
...
- (Package)packageRequestObject
{
    Package package = (id)^(id object,uint16_t seqNo)
    {
        ...
        [getMsgListReq setSessionId:[ZKUtil changeIDToOriginal:array[3]]];
